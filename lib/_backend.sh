...
backend_db_create() {
  ...
  sudo su - root <<EOF
  usermod -aG docker powertechdeploy
...
backend_chrome_install() {
  print_banner
  printf "${WHITE} 💻 Vamos atualizar o sistema PowerTech Pro+...${GRAY_LIGHT}"
  printf "\n\n"
...
backend_set_env() {
  print_banner
  printf "${WHITE} 💻 Configurando as variáveis de ambiente (backend)...${GRAY_LIGHT}"
  printf "\n\n"
...
sudo su - powertechdeploy << EOF
  cat <<[-]EOF > /home/powertechdeploy/zpro.io/backend/.env
NODE_ENV=
BACKEND_URL=${backend_url}
FRONTEND_URL=${frontend_url}
...
backend_node_dependencies() {
  print_banner
  printf "${WHITE} 💻 Instalando dependências do backend PowerTech Pro+...${GRAY_LIGHT}"
  printf "\n\n"
